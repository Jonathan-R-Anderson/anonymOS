...
function initPump() {
	...
	eventPump.addEventListener("new-client", function(e) {
		...
		eventPump.addEventListener("bump-thread", handleBumpThread);
...
function handleNewReply(e) {
	...
	let ports = threadSubscriptions.get(threadId);
	if (!ports) return;
	ports.forEach(function(port) {
		port.postMessage({"message": "new-reply", "thread": threadId, "reply_to": eventData.reply_to, "post": eventData.post});
	});
}
...
function handleBumpThread(e) {
	let eventData = JSON.parse(e.data);
	let boardId = eventData.board;
	let ports = boardSubscriptions.get(boardId);
	if (!ports) return;
	ports.forEach(function(port) {
		port.postMessage({"message": "bump-thread", "thread": eventData.thread, "board": boardId});
	});
}
