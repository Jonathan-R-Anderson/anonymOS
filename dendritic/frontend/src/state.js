...
const DEFAULT_STATE = {
    ...
    mediaDelivery: {},
    remoteReply: {},
    catalogInfo: {board: "", styles: []},
};

function reducer(state = DEFAULT_STATE, action) {
	switch(action.type) {
	case NEW_POST:
		if(state.posts.some(post => post.id == action.post.id)) {
			return state;
		}
...
export { newPost, newThread, deleteThread, setPosts, setThreads, setBoard, setStyles, setCaptcha, reducer, DEFAULT_STATE};
