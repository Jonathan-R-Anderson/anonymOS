...
std::string execAndGet(const char* cmd) {
    // HanonymOS: there is no /bin/sh and the referenced tools (lspci, addr2line,
    // nm) are absent, so this only ever produced "error"/"".  More importantly,
    // forking a /bin/sh from this multithreaded compositor deadlocks the child on
    // a mutex inherited locked from another thread (and our synchronous CProcess
    // then blocks forever polling its pipe), which stalls compositor startup
    // (SystemInfo::getSystemInfo runs right before the GL renderer is created).
