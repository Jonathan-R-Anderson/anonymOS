...
static const char* eglPlatformToString(EGLenum platform) {
    switch (platform) {
        case EGL_PLATFORM_DEVICE_EXT: return "device";
        case EGL_PLATFORM_GBM_KHR: return "gbm";
        case EGL_PLATFORM_SURFACELESS_MESA: return "surfaceless";
        default: return "unknown";
    }
}

void CHyprOpenGLImpl::initEGL(EGLenum platform, void* nativeDisplay) {
    ...
    if (m_eglDisplay == EGL_NO_DISPLAY)
        RASSERT(false, "EGL: failed to create a {} platform display", eglPlatformToString(platform));

    attrs.clear();

    EGLint major, minor;
    if (eglInitialize(m_eglDisplay, &major, &minor) == EGL_FALSE)
        RASSERT(false, "EGL: failed to initialize a {} platform display", eglPlatformToString(platform));

    Log::logger->log(Log::DEBUG, "EGL: initialized {} platform display, EGL {}.{}", eglPlatformToString(platform), major, minor);
    ...
    if (m_eglContext == EGL_NO_CONTEXT) {
        ...
        Log::logger->log(Log::WARN, "EGL: Failed to create a context with GLES3.2 on {} ({} / 0x{:x}), retrying 3.0", eglPlatformToString(platform),
                         eglErrorToString(err32), err32);
        ...
        if (m_eglContext == EGL_NO_CONTEXT) {
            const auto err30 = eglGetError();
            RASSERT(false, "EGL: failed to create a context with either GLES3.2 or 3.0 on {} ({} / 0x{:x})", eglPlatformToString(platform),
                    eglErrorToString(err30), err30);
        }
    }
...
CHyprOpenGLImpl::CHyprOpenGLImpl() : m_drmFD(g_pCompositor->m_drmRenderNode.fd >= 0 ? g_pCompositor->m_drmRenderNode.fd : g_pCompositor->m_drm.fd) {
    ...
    bool success = false;
    if (m_drmFD < 0 && EGLEXTENSIONS.contains("EGL_MESA_platform_surfaceless")) {
        Log::logger->log(Log::WARN, "EGL: no DRM fd, using surfaceless platform");
        success = true;
        initEGL(EGL_PLATFORM_SURFACELESS_MESA, EGL_DEFAULT_DISPLAY);
    }

    if (!success && EGLEXTENSIONS.contains("EGL_EXT_platform_device") && m_proc.eglQueryDevicesEXT && m_proc.eglQueryDeviceStringEXT) {
        m_eglDevice = eglDeviceFromDRMFD(m_drmFD);

        if (m_eglDevice != EGL_NO_DEVICE_EXT) {
            success = true;
            initEGL(EGL_PLATFORM_DEVICE_EXT, m_eglDevice);
        }
    }

    if (!success && m_drmFD >= 0) {
        Log::logger->log(Log::WARN, "EGL: EXT_platform_device unavailable for this DRM fd, using gbm");
        if (EGLEXTENSIONS.contains("EGL_KHR_platform_gbm") || EGLEXTENSIONS.contains("EGL_MESA_platform_gbm")) {
            m_gbmFD = CFileDescriptor{openRenderNode(m_drmFD)};
            if (!m_gbmFD.isValid()) {
                Log::logger->log(Log::ERR, "Couldn't open a gbm fd");
            } else {
                m_gbmDevice = gbm_create_device(m_gbmFD.get());
                if (!m_gbmDevice) {
                    Log::logger->log(Log::ERR, "Couldn't open a gbm device");
                } else {
                    success = true;
                    initEGL(EGL_PLATFORM_GBM_KHR, m_gbmDevice);
                }
            }
        }
    }

    if (!success && m_drmFD < 0 && !EGLEXTENSIONS.contains("EGL_MESA_platform_surfaceless")) {
        Log::logger->log(Log::ERR, "EGL: no DRM fd and EGL_MESA_platform_surfaceless is unavailable");
    }

    RASSERT(success, "EGL does not support a usable EGL_MESA_platform_surfaceless, EGL_KHR_platform_gbm, or EGL_EXT_platform_device path.");
