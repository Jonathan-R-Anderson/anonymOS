...
SP<ITexture> IHyprRenderer::getBackground(PHLMONITOR pMonitor) {
    ...
    // EpinAnonymOS: the wallpaper resource can finish "ready" but with a null
    // cairo surface if the PNG failed to decode (e.g. missing / unreadable on the
    // guest fs).  Guard against dereferencing it — otherwise getBackground()
    // segfaults the whole compositor instead of just skipping the background.
    if (!m_backgroundResource->m_asset.cairoSurface) {
        Log::logger->log(Log::ERR, "BGTex: background resource ready but cairo surface is null; skipping wallpaper");
    ...
    // EpinAnonymOS: warp straight to fully opaque instead of starting a 0→1 fade.
    // The fade relies on the animation manager ticking every frame; on the
    // damage-driven headless path the frame loop can idle mid-fade, leaving the
    // wallpaper at alpha 0 (invisible) over the background-color clear. No fade =
    // the wallpaper is visible on the very first composited frame.
    pMonitor->m_backgroundOpacity->setValueAndWarp(1.F);
