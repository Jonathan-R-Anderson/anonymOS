...
## GW3 — Run Weston as init + a terminal. P: Critical — DONE (2026-06-07); input + visible cursor (GW4) + snappy kernel cursor (GW5) DONE (2026-06-08)

**Goal:** Boot into Weston with a usable graphical terminal and shell.

**DONE (2026-06-07): THE FULL WESTON DESKTOP RENDERS THROUGH THE GW2 KMS — screenshot-verified.** A QMP `screendump` shows the desktop background (`0x1e1e2e`) + a top panel with a launcher icon and a live clock ("Thu Jan 01, 12:00 AM"), composited by Weston's Pixman renderer and pushed to `g_fb` by `drmPresentFb`. Two final fixes after the output came up: **(A)** `execve` dropped the real `envp` (fixed kernel env) so Weston's `WAYLAND_SOCKET` never reached the shell → "permission to bind desktop_shell denied"; fixed by snapshotting the caller's `envp` in `execveTask` before the CR3 switch and seeding via `linux_seed_initial_stack_with_args`. **(B)** flip-completion events were keyed to the flipping fd, but Weston PAGE_FLIPs on one fd and `epoll`s a *dup* — so it stalled after one frame; fixed by making the flip-event queue fd-agnostic (one card0 → any DRM fd drains it). **GW4 DONE (2026-06-08): INPUT + A VISIBLE, MOVING MOUSE CURSOR — screenshot-verified (`weston-ep.png`).** Three kernel bugs (each a general fix, all committed to `main`): **(1) in-kernel CoW/demand-zero page fault was unrecoverable** — `report_kernel_fault` only halted, so when the kernel was the first writer to a CoW / lazily-mapped user page (e.g. `socketBufferRead` copying recvmsg data into a freshly-mmap'd client buffer), the `#PF` at CPL 0 killed the box ("KERNEL FAULT"); fixed by resolving recoverable kernel page faults through `handlePageFault` and `iretq`-ing back (the `inKernelFault` stub restores GP regs and returns to the faulting instruction). This unblocked `weston-desktop-shell`. **(2) memfds couldn't grow** — `ftruncate` returned `EINVAL` when enlarging a backed memfd, so libwayland-cursor's incrementally-grown `wl_shm` cursor pool failed and the theme loaded **0** cursors ("could not load cursor 'left_ptr'" ×16) even though the kernel served the files byte-perfectly; fixed by making `ftruncate` grow the memfd (realloc+copy+repoint) and implementing `mremap` (was `ENOSYS`) for the `wl_shm` pool-growth case. The hardware-cursor ioctls now return `EINVAL` so Weston marks the cursor "broken" and Pixman-composites it into the primary framebuffer (our present path only scans out the primary plane). **(3) THE cursor blocker — nested epoll:** pointer motion reached the kernel evdev ring (PS/2 IRQ → `input_enqueue`) but Weston never read it. libinput hands Weston an epoll fd that itself watches the evdev devices, added to Weston's *outer* epoll; `fdReadable` had no `FD_EPOLL` case so an epoll fd always reported not-readable → the outer epoll never woke for the libinput fd → `libinput_dispatch` never ran. Fixed by making `fdReadable(FD_EPOLL)` return true when any watched fd is ready (recurses, depth-bounded). With it the pointer moves and the `left_ptr` arrow renders and tracks the mouse.

**GW5 DONE (2026-06-08): SNAPPY cursor — screenshot-verified (`ng-after.png`), commit `7ff6bb921`.** GW4's pointer was laggy/intermittent: with the Pixman software renderer there is no hardware cursor, so Weston re-composites the whole 1280×800 scene on every move and presents only intermittently. (Pixman also can't drive a hardware cursor *plane* — `drm_output_init_cursor_egl` is only wired into the EGL/gbm output path, never `drm_output_init_pixman`.) Fixed by drawing the cursor **in the kernel, decoupled from Weston's repaint**: **(1)** event1 is now an **absolute** pointer (`EV_ABS` + `ABS_X/ABS_Y` ranged to the framebuffer — the QEMU usb-tablet model); `handleMouseIRQ` accumulates the raw deltas into an on-screen position and reports the *same* absolute position to Weston, so Weston's pointer stays exactly aligned (clicks land correctly). **(2)** An 11×17 `left_ptr` sprite is stamped straight onto `g_fb` (with save-under) on every mouse IRQ and re-stamped after every present, so it tracks at interrupt rate and is visible at screen-centre from the first frame. **(3)** Weston's own software cursor is suppressed (`libweston/input.c` `pointer_set_cursor` forces the sprite NULL) so the overlay doesn't ghost it. **Trap that cost a bisect:** the present-time repaint runs in syscall context where interrupts are already masked — it must **not** `cli/sti`, since a stray `sti` unmasks interrupts mid-syscall and deadlocks the compositor's present path (the shell froze loading fonts, screen black). **Build note:** the `libweston/input.c` change needs `ninja -C deps/weston-14.0.0/build-epin libweston/libweston-14.so.0.0.0` before `make hos.iso`. **Cosmetic remaining:** the terminal launcher icon `.png` still won't decode (gio "error reading from input stream" — fallback box shown); `/etc/localtime` is empty (harmless `mmap(0)` from the clock's tz load).

**Earlier progress (2026-06-07): WESTON BOOTS AND PRESENTS VIA THE GW2 KMS.** Staging + launch done (all 8 modules as flat boot modules under `WESTON=1`; `weston-terminal` built with `-Dtools=terminal`; `init=weston` by exact basename; musl `ld` resolves everything by basename; `/etc/weston.ini` + `WESTON_MODULE_MAP` + `--drm-device=card0`). A full TCG smoke boot now reaches: load `drm-backend.so` → libseat opens `seat0` → opens `/dev/dri/card0` (SCM_RIGHTS fd pass) → **"DRM: does not support atomic modesetting"** (my `SET_CLIENT_CAP` reject → legacy KMS) → **"Using Pixman renderer"** → **"DRM: head 'HDMI-A-1' found, connector 1 connected"** (my GETCONNECTOR/GETENCODER) → **"Output 'HDMI-A-1' enabled"** (my SETCRTC present) → launches `weston-keyboard` + `weston-desktop-shell` → clients connect, allocate wl_shm buffers, load fonts, **draw** → Weston composites (Pixman) and **PAGE_FLIPs through my GW2 `drmPresentFb` → `g_fb`**. GW2 is proven end-to-end.

Getting here took **8 real kernel bugs** Weston exposed (Hyprland's headless path never hit them), all now fixed: (1) `signalfd4` ENOSYS stub → eventfd-backed (else libwayland aborts Weston silently at startup); (2) `fcntl F_SETFL` wrongly required `CAP_RIGHT_WRITE` → EBADF on a pipe read-end → broke libseat's seatd `poller_init`; (3)+(4) udev DRM discovery (`/sys/class/drm/card0` synthetic dir + `card0/uevent`); (5) **`SEATD_VTBOUND=0`** env — a VT-bound seat never activates with no `/dev/tty0`, so seatd refused device opens (EPERM "client is not active"); (6) **synthetic-dir shim shadowed the uevent FILE** — `isSyntheticDirectoryPath()` returned true for `…/card0/uevent` (prefix fallback), serving it as an empty dir before the g_vfs table → udev got no DEVNAME; fixed by guarding the synth-dir branch with `!pathIsExactVfsFile(path)`; (7) **`AF_NETLINK` socket** — libudev-zero's udev monitor needs it (else "failed to create input devices", fatal); (8) **`fallocate` (285)** unimplemented — weston's wl_shm allocator (`memfd_create`→`posix_fallocate`) failed so no client could draw. (The earlier hypothesis that the libseat device-open *socketpair recvmsg* was broken was **wrong** — that path is fine; #5 and #6 were the real cause, and #6 is why no socket request was ever sent.)

**ONE item left for the full desktop:** `weston_desktop_shell: permission to bind desktop_shell denied` — the panel/background client can't bind the privileged shell global because our `execve` uses a **fixed kernel env and drops the real `envp`**, so Weston's `WAYLAND_SOCKET=<fd>` (which makes the spawned shell reuse the trusted, pre-created `wl_client`) is lost and it connects fresh via `WAYLAND_DISPLAY` → a different client → denied. Normal clients draw fine; only the privileged shell needs same-connection trust. **Next:** capture the real `envp` in `execveTask` *before* the CR3 switch and pass it to the stack seeder (so `WAYLAND_SOCKET` + the inherited `sv[1]` fd survive); evaluate the existing `_seedLinuxStack`. Risk: `execve` is the critical boot path. Best iterated on the KVM host (TCG here is ~8× slowed by 7 orphaned qemu processes — worth cleaning up).
