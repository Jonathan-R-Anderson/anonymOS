...
| 4.8 | ⚠ **The state split is three subtrees, not a default.** Surfaced by 4.2's `IMMUTABLE-2` gate, which still reports FAIL by design. `storeMountSystem()` builds the system namespace with `nsAlloc()`, whose `bindRoot()` binds `/` with `rights = uint.max`, so `/usr` is genuinely read-only (its own binding is more specific) but every path **outside** the three explicit bindings inherits write rights from that catch-all root. This is §G mistake #3, ambient namespace. `nsAllocRestricted()` exists for exactly this and omits the root binding — but switching to it denies `/dev`, `/proc`, `/run` and `/home` to task 0 and everything inheriting from it, so it needs its own staged proof rather than a one-line change. Until it holds, "immutable" is honestly **read-mostly** (2/4) | IMMUTABLE_ROOTLESS §F/§G |
| 4.9 | **Runtime signed policy transactions.** The blocker shared by 4.1's disposable identities and 4.3's Phase 9: `identityCreate()` refuses once `g_idFrozen`, which `configboot` sets after loading policy — a security property, not an oversight, since privilege cannot then be minted at runtime. Identity policy is therefore declarative **at boot only**. Live reconfiguration needs the §9 path: a `policyEpoch` bump under `CAP_RIGHT_ADMIN_IDENTITY`, verified like `update.d`'s inactive-slot pattern so a bad policy cannot brick the running set. Also outstanding from 4.3: the manifest's `dh` and `audit` IPC flags are emitted and the kernel ignores them | IDENTITY_DOMAIN §9 · DECLARATIVE_CONFIG |
| 4.10 | **A/B needs a second physical ESP (D2) and real release signing (D3).** 4.4 built and proved the update *unit*, but staging verifies and picks the target slot **without copying bytes**, because the installer still lays down ONE ESP — there is no ESP-B to stream into. D2 adds ESP-A/ESP-B to the GPT (the boot-state sector and `efi_arbiter.c` already exist). D3 replaces the **symmetric** HMAC signature: whoever holds `scripts/mk-hosupd.sh` can forge a bundle this kernel accepts, which is fine for a local image and is not a release-signing story. The bundle already carries `keyId` so Ed25519 fits without a format break | SYSTEM_UPDATE D2/D3 |
| 4.11 | **Rebuild deps with guest-relative prefixes.** 4.7 removed the builder's identity from the ISO with a same-length in-place sanitiser, which is a build-output fix. The principled one is `--prefix`/sysroot so musl, Mesa, glib, fontconfig and zsh never bake host paths in at all — then nothing needs sanitising and the guest's own path probes could actually resolve instead of failing `ENOENT` | DECOY_SECURITY |
| 4.12 | **Full read isolation needs a filesystem-layout change.** 4.0b confinement enforces read-mostly, write-confined, explicit denies — but `allowTraversalOutsideMounts` grants read-only `/` because the binaries live at the root, so a confined app can still *read* broadly. Moving executables under a proper prefix is what closes it | IDENTITY_DOMAIN |
