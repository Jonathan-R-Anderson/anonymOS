...
| 3.2 | ◑ **Multi-window and workspace experience** — **UNBLOCKED 2026-09-06**: `wl-calendar`, `wl-logview` and `wl-wifi-menu` all reflow and tile — **verified on screen**, each re-reflowing at more than one width. 2 left (`wl-overview`, `wl-quicksettings`) plus `wl-domain-manager`, each needing its own layout reflow before `set_max_size` can go | Hyprland provides the mechanism; this is the desktop actually using it | GUI G20 | M |
