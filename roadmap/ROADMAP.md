...
| 3.2 | ◑ **Multi-window and workspace experience** — **UNBLOCKED 2026-09-06**: `wl-calendar` and `wl-logview` reflow and tile (verified: 4 windows tiled, no overlap, both re-reflowing at a second width). 3 clients left (`wl-overview`, `wl-wifi-menu`, `wl-quicksettings`) plus `wl-domain-manager`, each needing its own layout reflow before `set_max_size` can go | Hyprland provides the mechanism; this is the desktop actually using it | GUI G20 | M |
