...
| 4.1 | ◑ **Identity brokers, disposable identities, policy engine (Phases 7–9).** All three existed as code only a SELF-TEST had ever reached. **Done 2026-09-06:** (a) two identities now run concurrently — `wl-calc` spawned into BankVault via `domainSpawnInto` runs Banking (yellow border) beside the Personal session (green), which was the precondition everything else needed; (b) the **cross-identity IPC gate is live** — `idipcMayConnect()` consulted on every AF_UNIX connect, the first non-self-test caller of `g_idIpcRules`. **Rule derived from measurement:** every real cross-identity connect is app -> compositor on the Wayland socket and there is **no** app-to-app crossing, so a system-trust peer is a **hub** any identity may reach (refusing it would cut every app off its display) and app-to-app is `EACCES` without a broker pair rule. **Remaining:** the DENY path is unexercised (nothing performs an app-to-app cross-identity connect yet); **disposable identities have no runtime creation path** (`disposable` is set only for boot identities and self-test fixtures); the broker session-descriptor flow (`brokerRequestSession`) is still self-test-only | IDENTITY_DOMAIN |
