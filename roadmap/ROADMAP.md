...
| 4.8 | **Prove §F 4/4 on a REAL installed system.** `tests/immutable-installed.txt` reaches `immutable 4/4 -> IMMUTABLE; rootless 4/4 -> ROOTLESS` on a disk-backed store, but it gets there by giving the QEMU fixture a bare GPT — not by running the installer and booting the result. The gap is small and worth closing exactly once: install to a disk, boot the installed system, and read the summary line there. Until then "immutable" is proven on a store that mounts, not on an OS that was installed | IMMUTABLE_ROOTLESS |
