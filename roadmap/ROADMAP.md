...
|---|---|---|---|---|
| 3.5b | ◑ **Nothing parks on an idle desktop — root cause found, half fixed.** `wakePollers()` ran on every PIT tick and un-parked **every** poll-blocked task unconditionally, so a task with nothing ready was woken at 1 kHz, re-scanned, found nothing and re-parked: ~7 tasks x ~1000 pointless syscall round-trips/sec with `[fdrd]` showing 2 fds actually ready. **Epoll waiters fixed** (186e2f5b60): Hyprland ~1000 -> 13-159/sec, dbus-daemon ~1000 -> 11-157, desktop healthy, golden clean. **`poll()` waiters still spin** — the same fix there did not boot (reverted d37fb65e90): the blanket wake is a system-wide safety net, and removing it left a futex waiter spinning on `*u == val` forever. Next: find what depends on the blanket wake, starting from that futex, rather than removing the net and seeing what falls | DESKTOP_RESP | M |
