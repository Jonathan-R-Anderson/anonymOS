...
| 4.2 | ✅ **Immutable-rootless foundation (Phase 0 invariants) — DONE 2026-09-07.** §0.4 asked for "a test list that **fails today** and must pass to claim each property"; §F's eight conditions had lived as prose, so the only way to answer "is this system immutable?" was to read the source and form an opinion. `core/acceptance.d` now answers it **every boot, in eight lines, each printing the probe that decided it** — and every probe exercises a path where a wrong implementation gives the other answer (an admin right demanded from a table never given it; a derive asking for more than the parent holds; rollback measured by reading `genActive()` back; W+X pushed through the real PTE builder, with exec-alone checked too so a blanket-NX kernel cannot pass by refusing everything). **Result: rootless 4/4 → ROOTLESS** (uid 0 grants nothing, one admin right grants exactly that right, widening refused + revoke strips authority, PID1 lacks update/user/device). **Immutable 2/4 → "read-mostly"**: `IMMUTABLE-3` atomic update+rollback and `IMMUTABLE-4` W^X pass. **The gate found a real defect on its first run:** `storeMountSystem()` uses `nsAlloc()`, whose `bindRoot()` binds `/` with `rights = uint.max`, so `/usr` is genuinely read-only but every path outside the three explicit bindings inherits write rights — the split is three named subtrees, **not** a default (§G #3, ambient namespace). Not fixed here on purpose: `nsAllocRestricted()` is the fix but denies `/dev`, `/proc`, `/run`, `/home` to task 0 and everything inheriting from it, so it needs its own staged proof. `IMMUTABLE-1` fails only on live media (`backing-on-disk=0`, by design — the disk is reserved for the installer). **0.3 spec written** (`roadmap/IMMUTABLE_LAYOUT_SPEC.md`: the three trees, A/B slots, on-disk store layout, both known gaps); **0.2** is covered by `CAPABILITY_MODEL.md` (`ObjHeader`, rights, untyped retype). **Proof:** `tests/immutable-rootless.txt` — PASS; the four earned ROOTLESS gates are `require`d so a regression fails the build, and the two unearned IMMUTABLE gates are asserted **still failing**, so a property can only be claimed deliberately | IMMUTABLE_ROOTLESS |
| 4.3 | ✅ **Declarative config compiler (Phases 1–9) — DONE 2026-09-07.** The host compiler was never the problem: all eight stages (~3,000 lines) already parsed, validated, resolved refs, detected cycles over four edge sets, checked capability subsets and assigned deterministic ids. **The LOWERING was the problem — the compiler computed policy the kernel never received.** (a) **Phase 5/9 — identity policy was inert.** `manifest.d` never serialised `net`/`clip`/`gui` (so configboot hardcoded `NetPolicy.None`/`ClipPolicy.Deny`/`gui=0`), and configboot treated an already-existing identity as a no-op "the config agrees with the built-ins" — but nothing checked that it agreed, and it did not: `system.json` declared Personal `#3478F6` while the screen painted `0xFF2E7D32`. **Every one of the four "applied" identities took the no-op branch, every boot.** Fixed by appending the three fields (old blobs still parse — no version bump) and adding `identityApplyPolicy()`, which refuses once `g_idFrozen`, making the window exactly "a verified manifest, before the seal" — the signed-policy-transaction path IDENTITY_DOMAIN §2 already sanctions. **Proven by experiment:** Personal set to `#00E5FF` → the census reported `color=00000000ff00e5ff` while Banking and Disposable stayed put; reverted. (b) **Phase 7 — declared IPC rules were inert too.** `ipc[].allow[]` compiled into `g.ipcRules` and stopped there, so `g_idIpcRules` held only what `idipcInit` hardcoded. This matters more since 4.1 made those rules gate **every cross-identity AF_UNIX connect**. Broker crosses the wire **by name** (a service objId does not exist until registration) and resolves via `serviceLookup` — proven live: `broker=0x67`, not 0. **`system.json` was aligned to the values the system actually uses**, so behaviour is unchanged and the file finally states the truth. Trust levels were verified identical first — 4.1's hub rule keys off `trust >= TRUST_SYSTEM` and a mismatch would have cut every app off its display. **Proof:** `tests/disposable-identity.txt` asserts the **apply lines**, not just the painted colour — a colour can match by coincidence when table and manifest agree, which is precisely the state this tier found. **Not done:** the `dh`/`audit` flags are emitted but the kernel ignores them; Phase 9's **live-reconfiguration** half (§13) needs the runtime signed-policy-transaction machinery 4.1 also left unbuilt (`policyEpoch` bump + `CAP_RIGHT_ADMIN_IDENTITY`), so identity policy is declarative **at boot**, not live | DECLARATIVE_CONFIG_SPEC |
| 4.4 | ✅ **Whole-image A/B update unit (D1) — DONE 2026-09-07.** Everything AROUND the update already existed — `bootstate.d` holds the boot-state sector, `efi_arbiter.c` chainloads by it, `sysupdate.d` drives switch/rollback/boot-ok — but the artifact they are all about did not. `sysupdate.d` says so in its own header: *"no image staging yet"*. `core/update.d` is not it either: its unit is a generation of content-addressed store objects, whereas D1's is a whole ESP image verified as one hash. **New `core/imgupdate.d`** signs a 64-byte header that commits to the payload by SHA-256, so one signature check plus one hash authenticates the artifact — and the hash can be computed while streaming, which is what U2's chunked download needs. **Verification order is deliberate:** shape → signature → the hash the now-trusted header commits to → policy. Anti-rollback is checked **last**, because a replayed release is validly signed with a correct hash and fails only on policy; checking it earlier would report a misleading reason for a real attack. Each refusal returns its **own** verdict — "someone flipped a bit" and "someone replayed v1" need different responses. **`scripts/mk-hosupd.sh`** packages a real `.hosupd`, and the ISO stages one so the kernel verifies a bundle the HOST produced: a self-test cannot catch a host/kernel wire-format disagreement because both its sides are the same code, and that disagreement is the failure this project actually hit twice on the config manifest. **Found and closed a data-loss hazard:** D1's "user data is untouched" rested on `GPT_GAP_FIRST`(40) > `BOOTSTATE_LBA`(34) — two constants in two modules linked only by a comment, with a third copy in `efi_arbiter.c`. Lowering either would put the object store's superblock exactly on the sector every update rewrites, and a correctly-signed update would silently eat user data; no signature check catches that. Now a `static assert`, **verified to fire** by setting it to 34 (build fails). **Proof:** `tests/image-update.txt` — PASS; all five verdicts asserted by name. **Not claimed:** D2's second physical ESP does not exist (the installer lays down one), so staging verifies and picks the target slot but copies no bytes; D3's Ed25519 is absent — signing uses the **symmetric** HMAC key, honest for a local image and *not* a release-signing story (the bundle carries `keyId` so the swap needs no format break) | SYSTEM_UPDATE D1 |
| 4.5 | ◑ **Native object shell — B5 audit gap CLOSED 2026-09-07; the row's old "remaining" list was stale.** The implementation diverged from the B-track doc and is further along than it read: the native shell is not a POSIX parser with pipes (B2) but an **LFE (Lisp-Flavored Erlang) interpreter** in `hos-sh.d` — cell-based values, closures, `defun`/`lambda`/`let`, `if`/`case` with pattern matching and guards, recursion — so B2's "pipes + if/for/while" is moot by design. B4 also landed differently: `EPIN_SHELL=native` launches **`/hos-zsh`** (one zsh, two personalities) rather than a standalone `-sh`. B3's mutating verbs exist in the ABI *and* the shell references all of them. **The one REAL gap was B5's "audit-log every privileged action", and it was total:** the native object ABI's four mutating verbs — `cap_grant`, `ns_clone`, `ns_enter`, `id_switch` — wrote **no** audit record, and `hoscall.d` did not import `core.audit` at all. In a system whose security model *is* capabilities plus identity, the operations that move both were the only ones invisible to the log, while `requireCap` decisions, cross-identity IPC refusals and namespace denials had been audited for tiers. Now wrapped at the **dispatch site** (one place; cannot be forgotten when a fifth verb joins) recording the **outcome**, so a refused grant is as visible as a granted one. New `AuditKind`s **appended, never inserted** — inserting renumbers every later kind and silently changes the meaning of captured records. **Proof:** `tests/native-verb-audit.txt` PASS — `ns_clone=0x7e auditOk +1`, `ns_enter(bogus)=-1 auditDeny +1`; the **denial** is the load-bearing assertion, since a trail recording only successes is the one an attacker is content to leave. **Remaining (B5 polish only):** confirmations + `--dry-run` on destructive ops, `help`/`man` builtins, tab-completion — deliberately not attempted, because they are interactive and cannot be verified headlessly, and this tier's standard is measured proof rather than shipped-and-assumed | SHELL_AND_COMMANDS |
...
| 4.7 | ✅ **The shipped ISO no longer names its builder — DONE 2026-09-07.** Was **396** occurrences of `/home/bruns/Documents/anonymOS` across 28 files; now **0**, and `/home/bruns` appears **0** times anywhere in the image. Nothing was ever broken by it — the guest probes those paths and every one fails `ENOENT` — which is exactly why it survived: an image whose premise is deniability shipped its builder's username and source-tree layout in every copy anyone would distribute. `scripts/sanitize-build-paths.sh` does an **in-place, same-length** byte replacement, which is what makes it safe on ELF (nothing shifts, so offsets, section sizes and relocations stay valid); padding is repeated slashes, which POSIX collapses, so `…/anonymOS/deps/musl/…` becomes `/build//…//deps/musl/…` and resolves as `/build/deps/musl/…`. **Two passes, longest prefix first:** the build root, then `$HOME` — because the first pass alone left **114** behind, from binaries vendored as PREBUILT artifacts carrying `__FILE__` strings from an older checkout (`…/Documents/EpinAnonymOS/deps/gnupg…`) that no longer exists; a stale path leaks the username just as well as a live one. Runs **before** `build-boot-integrity-manifest.py`, which hashes `cd/` — sanitising after would invalidate every hash in the attestation. Fails the build rather than passing quietly if any occurrence survives. **Verified:** 396→0, `/home/bruns`→0, and the sanitized ISO boots (`desktop-smoke` PASS), so patching 32 binaries broke nothing. **Not claimed:** the principled fix is rebuilding deps with guest-relative prefixes so the strings are never baked in; this is a build-output sanitiser. Cross-machine byte-identical output follows from removing the only path-dependent bytes, but was not measured by building in two directories | DECOY_SECURITY · IMMUTABLE_ROOTLESS |
