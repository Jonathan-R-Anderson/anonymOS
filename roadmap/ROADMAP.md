...
| 4.0b | ◑ **Namespace confinement — enforcement OFF (reverted 2026-09-06, it broke installing).** The bind works and was proven enforcing (`CONFINED`, distinct ns clone per task, desktop up, 0 faults) — but the installer is spawned through the same path and the policy denies `/config`, which it opens for `disks.json`, so **the OS could not be installed**. Predicted in writing and shipped anyway because the desktop still booted; booting is not the test for a change that gates file access, and the golden check screenshots the installer BEFORE disk enumeration so it cannot catch it. **Per-app policy DONE 2026-09-06** — `isSystemProgram()` allowlist (installer, domain manager, Hyprland, bar, `hos-*`); unknown names fail safe as unconfined; verified installer + domain-manager `/config` reads succeed, 0 faults. **Remaining: SCOPE.** The hook is in `spawnWaylandProgram`, which only sees KERNEL-spawned tasks — apps launched from a keybind or the app menu are forked by Hyprland via `execve` and bypass it entirely, so on live media nothing is confined at all and on an installed system only autostart apps are. Covering user-launched apps means hooking `execve`, the hot path for every exec. Policy itself is measured-good (257/270 coverage) and identity assignment (4.0) stays on. What it enforced when on: Spawned tasks are bound into the session domain via `domainBindTaskNs()`; proven enforcing rather than inferred from quiet logs — `bind tid=2 domain=0x46 ns=0xa0 taskNs=0xa0 domainObjId=0x46 CONFINED`, a distinct namespace clone per task, desktop up, 0 faults, 0 denials (policy correct, not absent). **What it enforces, precisely: READ-mostly, WRITE-confined, explicit denies** (`/home/user/.ssh`, `/objects`). `allowTraversalOutsideMounts` adds a read-only `/` because this OS stages executables and libraries at the root — full READ isolation needs a filesystem-layout change and is NOT done. Got here by rewriting the policy against measured reality (coverage 0 -> 257 of 270 opened paths); two earlier attempts to instrument confinement from inside `open()` broke the boot and were reverted | IDENTITY_DOMAIN |
