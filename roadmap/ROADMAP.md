...
| 4.0b | ⚠ **Namespace confinement — identity is applied, isolation is NOT.** 4.0 labels a process with its domain's identity but does not apply the domain's restricted namespace. `domainBindTaskNs()` (task.d:375) does both together and its second half is the confinement — the boot proof calls it "restricted ns … (opens enforced)". So today a window shows Personal while the process can still open anything: **the label is honest about OWNERSHIP, not ISOLATION, and must not be read as the latter.** Work: bind spawned tasks into the session domain's namespace, which needs per-app policy (the compositor and installer legitimately need paths a confined app should not have), and a way to see what a confinement denial broke **BLOCKER, found 2026-09-06 by host-side analysis (zero kernel risk):** the declarative policy describes a namespace **this OS does not have**. DevSandbox allows `/Domains/DevSandbox/Home`, `/tmp`, `/Shared/Projects`, `/System/Templates`, `/Shared/Public` — and of the **270 distinct absolute paths the desktop opens in a boot, 0 fall under any of them**; `/Domains`, `/Shared` and `/System/Templates` are opened zero times because they do not exist. The real traffic is `/usr/share` (143 — fonts/icons/themes), `/home/user` (34), `/sys/class` (19), `/var/cache` (10), `/etc/glib-2.0`, `/run/user` (the Wayland socket), `/dev/dri`, `/dev/input`. **Policy rewritten 2026-09-06 and measured: coverage 0/270 -> 257/270 (95%%), desktop up, 0 faults.** Remaining 13: **9** are 4.7 build-host paths (already `ENOENT`), **2** are `/config/*` denied on purpose, and **2** are real open questions — (i) `//libdbus-1.so.3`: this OS stages binaries and libraries at `/`, so a deny-by-default namespace has nowhere to bind them without effectively binding `/`; (ii) `/home/.machine-id`: the kernel reads it while synthesising `/etc/machine-id`, and that read runs **in the calling task's namespace**, so confinement would break machine-id for a confined app. Both are design questions, not missing bindings. **Enforcement is still OFF** — nothing binds tasks into the namespace. **Attempt 1 FAILED and is reverted; read this before trying again.** (a) The desktop **cannot** simply be bound into the session domain: that view denies `/system`, where the compositor, installer and every app read fonts/config/binaries. (b) Audit-before-enforce is still the right approach, but **not inside `open()`** — a hook in `namespaceCheckOpen` that returned on its FIRST LINE still broke the boot with non-deterministic faults (different tasks, different addresses, dying in ld-musl); only removing it entirely restored the desktop, which points at the extra call frame on that hot path, not at logic. (c) A per-spawn `nsClone()` is also suspect and unproven either way. **Already collected, still valid:** `/etc/dbus-1/system.conf`, `/etc/machine-id`, `/home/.machine-id`, `/var/lib/dbus/machine-id`, `/run/sshd.log` are unbound in that view | IDENTITY_DOMAIN |
| 4.0c | ⚠ **Border drawn by the kernel, not the "trusted compositor".** IDENTITY_DOMAIN §56 specifies the compositor reads the window's `identityObjId` and draws the border; `compositor.d:644 borderColorFor()` implements exactly that and is **unused**. What actually paints is `drmSetHosWindows` -> `fbDrawBorder` in the kernel. Arguably stronger — the kernel is the TCB and an app cannot race it — but it is not what the document describes, and having two border paths where one is dead is how the `pid % 8` hash survived unnoticed. Work: decide which path is authoritative, delete or wire the other, and make the document match | IDENTITY_DOMAIN |
