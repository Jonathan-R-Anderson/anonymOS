...
| 4.8 | **Two leftovers from the §F gates and the config lowering.** (a) `IMMUTABLE-1` still reports FAIL, and on live media that is the CORRECT answer — the object store stays in RAM because the disk is reserved for the installer — but the gate is *expected* to pass on an **installed** system and that has never been verified; booting an installed disk and reading the line is the whole task. (b) From 4.3: the manifest carries the IPC policy's `dh` and `audit` flags and the kernel **ignores** them, so `"dh": true` in `system.json` grants nothing. Neither blocks anything; both are places the system currently claims slightly more than it does | IMMUTABLE_ROOTLESS · DECLARATIVE_CONFIG |
