...
| # | Item | Source |
|---|---|---|
| 5.0 | ✅ **VirtIO block driver — DONE 2026-09-07.** Proxmox VE defaults to VirtIO SCSI and `-drive if=virtio` is the usual way anyone attaches a QEMU disk, so a stock VM reported "no disk to install to" and the object store silently stayed in RAM — nothing broken, simply no device the kernel could see. New `drivers/block/virtio_blk.d` mirrors the proven `virtio_net.d` transport (same PCI capability walk, same common-config offsets, same rings-from-`alloc_phys_page`, same poll-the-used-ring) so any bug in it is a block-layer bug rather than a virtio one. Negotiates **only** `VERSION_1`, keeping every request a fixed three-descriptor chain on a 512-byte-sector device — exactly what the disk layer assumes. Probed **last**, so real AHCI/NVMe still wins. **Proof:** `tests/virtio-blk.txt` PASS via `VIRTIO_BLK=1 scripts/boot-test.sh` — device found at 00:04.0, capacity `0x400000` matching the 2 GiB image, and `[disk] selftest PASS (sector round-trip verified)`, which is the assertion that matters: enumeration only proves the PCI walk, the round-trip proves it moves data. The suite also **forbids** the AHCI ready line, so it cannot pass with AHCI quietly answering instead | — |
| 5.1 | ◑ **LKL hardware bridge — capability 1 of 4 DONE 2026-09-07.** L3 needs four things from the host, and userspace could reach **none** of them: `pciConfigRead32` existed but nothing exposed it, so a userspace LKL could not enumerate or program a device at all. ✅ **PCI config r/w** now exists as `HOSQ_PCI_CFG_RD/WR`, gated on `CAP_RIGHT_ADMIN_DEVICE` — which PID1 deliberately does **not** hold, so it is never ambient — and audited by 4.5s wrapper including refusals. Proven against an *independent* source (`tests/pci-verb.txt`): the verb reads `0x8086:0x1237` and the kernels own PCI scan agrees, an unauthorised caller gets `EPERM`, a misaligned offset gets `EINVAL`. **Remaining 3 of 4:** MMIO r/w at a physical address (BAR access via LKLs iomem forward — the roadmap notes this needs no mmap, the backend just forwards each MMIO), virt->phys for DMA (no IOMMU, so IOVA = phys), and IRQ forwarding — which `BARE_METAL` itself defers behind proving a polled driver first. **Then** L4 per-device isolation, and only after that is L5/USB or L6/GPU meaningful | BARE_METAL L3, L4 |
