...
| 4.12 | ◑ **Read isolation is ENFORCED where policy declares it — measured 2026-09-07; the remaining gap is a design choice, not a bug.** The old row said full read isolation "needs a filesystem-layout change" and left it at "read-mostly", which skipped the question that decides whether any of it works: does a policy's DENY survive the blanket read-only `/` that `allowTraversalOutsideMounts` adds? It does. `tests/read-isolation.txt` asks the live resolver and every answer matches policy — Throwaway is refused `/home/user/secret` and `/config/system.json`, DevSandbox is refused `~/.ssh/id_rsa` and `/config/system.json`, and `/usr/share/icons` stays readable so the probe demonstrably reports both answers. So the denies are real, not decorative, and Banking data is not readable from a Disposable window. **What remains** is that reads are a DENY-list over a blanket allow, rather than an allow-list: a path nobody thought to deny is readable. Closing that means the binaries stop living at `/` so the blanket read is unnecessary — a real ISO restructure that breaks every exec path referencing `/wl-calc` and friends, and worth doing deliberately rather than as a tier cleanup | IDENTITY_DOMAIN |
