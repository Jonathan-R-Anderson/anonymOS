module anonymos.kernel.shell_integration;

import sh_metadata : shRepositoryPath, shBinaryName, shRevision, shSourceFileCount,
    shDocumentedCommandCount, shBinarySizeBytes;

import anonymos.console : putChar, print, printLine, printCString, printUnsigned,
    printDivider, printStageHeader, printStatus, printStatusValue, clearActiveStage,
    stageSummaryData;
import anonymos.compiler : compileStage, frontEndSources, optimizerSources,
    runtimeSources;
import anonymos.syscalls.posix : ProcessEntry, launchInteractiveShell, shellExecEntry,
...
import anonymos.toolchain : resetBuilderState, configureToolchain, linkCompiler,
    packageArtifacts, toolchainConfiguration, linkArtifacts, packageManifest,
    linkedArtifactSize;
import anonymos.kernel.posixbundle : compileEmbeddedPosixUtilities;


// Import userland bootstrap functions directly
import anonymos.userland : UserlandRuntime, SystemProperties, normaliseState, 
    ...
    // Inline userland bootstrap (avoiding LDC betterC extern(C) dead code elimination)
    {
        ...
        if (!g_servicePlansInitialized)
        {
            ...
            g_servicePlans[9] = ServicePlan("terminal", "/bin/terminal", "ZSH Terminal Emulator",
                          [ "display.x11", "posix.exec", "ipc.userland" ], "ready", true);
...
// POSIX-in-Kernel shim implementation now lives in anonymos.posix::PosixKernelShim.
