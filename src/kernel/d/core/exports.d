    ...
    ulong pg)
{
    ...
    if (isHyprlandC) {
        ...
        if (envVirt != 0) envVirts[envc++] = envVirt;
        // Point Hyprland straight at our kernel-synthesized hyprlang config.  Its config resolver
        // (Jeremy::getMainConfigPath) PREFERS ~/.config/hypr/hyprland.lua and only probes /etc/hypr/
        // for .lua — it never finds our /etc/hypr/hyprland.conf, so it silently boots an AUTOGENERATED
        // default (wallpaper ON -> the CAsyncResourceGatherer decodes wallN.png and the worker thread
        // dies in mallocng; exec-once=/wl-layer-bar never runs -> no top bar).  HYPRLAND_CONFIG=<path>
        // is honored as CONFIG_TYPE_EXPLICIT and, since the extension is .conf (not .lua), loads via the
        // Legacy hyprlang parser -> disable_hyprland_logo=true skips the wallpaper entirely + our
        // exec-once/binds apply.
        envVirt = _copyKernelStrToStack(stackPhysVirt, stackVirtBase, strCursor, "HYPRLAND_CONFIG=/etc/hypr/hyprland.conf\0".ptr);
        if (envVirt != 0) envVirts[envc++] = envVirt;
    }
    if (!isCompositorGpu) {
