...
uint canvasText(ref Canvas canvas, const(FontStack)* stack, uint x, uint y, const(char)[] text, uint fg, uint bg, bool opaqueBg = true) @nogc nothrow
{
    ...
    {
        // Try to use the active font stack as fallback
        auto fallbackStack = activeFontStack();
        if (fallbackStack !is null && fontStackReady(fallbackStack))
        {
            stack = fallbackStack;
        }
        else
...
// Wrappers for Interface Compatibility
public Canvas canvasInit() @nogc nothrow
{
    return createFramebufferCanvas();
}

public void canvasClear(ref Canvas canvas, uint color) @nogc nothrow
{
    canvasFill(canvas, color);
}

public void canvasPixel(ref Canvas canvas, uint x, uint y, uint color) @nogc nothrow
{
    // Use canvasRect 1x1 as putPixel is private
    canvasRect(canvas, x, y, 1, 1, color, true);
}
