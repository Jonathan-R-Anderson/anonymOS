...
struct DisplayServerConfig
{
    ...
    /// When true, attempt to spawn the preferred GNOME/Mutter handoff target.
    /// `init` remains the authoritative boot path, but the kernel-side
    /// bootstrap state should point at the same desktop target ordering.
    bool gnomeShellEnabled         = true;
...
DisplayServerState bootstrapDisplayServer(DisplayServerConfig config) @nogc nothrow
{
    ...
    {
        state.compositorPid = startCompositorProcess(config);
...
private pid_t startCompositorProcess(DisplayServerConfig config) @nogc nothrow
{
    if (config.gnomeShellEnabled)
    {
        return spawnRegisteredProcess("/sbin/mutter", null, null);
    }
