...
struct USBHIDSubsystem
{
    ...
    private bool initializeController()
    {
        ...
        switch (controllerType)
        {
            ...
            default:
                print("[usb-hid] UNKNOWN controllerType = ");
                printUnsigned(controllerType);
                printLine("");
                return false;
    ...
    private void enumerateHIDDevices()
    {
        ...
        switch (controllerType)
        {
            ...
            default:
                print("[usb-hid] UNKNOWN controllerType in enumerate = ");
                printUnsigned(controllerType);
                printLine("");
                break;
        }
...
private __gshared bool g_ps2Break = false;
private __gshared HIDKeyboardReport g_ps2KeyboardReport;
private __gshared ubyte[4] g_ps2MousePacket;
...
private __gshared uint g_ps2PollThrottle = 0;
...
private void handlePs2KeyboardByte(ubyte data, ref InputQueue queue) @nogc nothrow
{
    ...
    if (data == 0xF0)
    {
        printLine("[ps2] Set 2 Break detected!");
        g_ps2Break = true;
        return;
    }

    const bool breakCode = g_ps2Break || ((data & 0x80) != 0);
...
private __gshared bool g_ps2MouseHasWheel = false;
private __gshared bool g_ps2MouseHas5Buttons = false;

private void ps2SetSampleRate(ubyte rate) @nogc nothrow
{
    ps2WriteCommand(0xD4);
    ps2WriteData(0xF3); // Set Sample Rate
    if (ps2WaitOutputFull()) cast(void)inb(ps2DataPort); // ACK
...
private void enableIntellimouse() @nogc nothrow
{
    // Magic sequence: Set Sample Rate 200, 100, 80
    ps2SetSampleRate(200);
    ps2SetSampleRate(100);
    ps2SetSampleRate(80);
    
    // Get Device ID
    ps2WriteCommand(0xD4);
    ps2WriteData(0xF2); // Get Device ID
    if (ps2WaitOutputFull()) cast(void)inb(ps2DataPort); // ACK
    
    if (ps2WaitOutputFull())
    {
        ubyte id = inb(ps2DataPort);
        if (id == 0x03)
        {
            printLine("[usb-hid] Intellimouse (Wheel) enabled!");
            g_ps2MouseHasWheel = true;
        }
        else if (id == 0x04)
        {
            printLine("[usb-hid] Intellimouse Explorer (5-button) enabled!");
            g_ps2MouseHasWheel = true;
            g_ps2MouseHas5Buttons = true;
        }
    }
}
...
private void handlePs2MouseByte(ubyte data, ref InputQueue queue) @nogc nothrow
{
    ...
    // Sync check for first byte (Bit 3 must be 1)
    if (g_ps2MouseIndex == 0 && (data & 0x08) == 0)
    {
        // printLine("[usb-hid] PS/2 mouse out of sync");
        return;
    }

    g_ps2MousePacket[g_ps2MouseIndex++] = data;
    
    // Packet size depends on mode
    uint packetSize = g_ps2MouseHasWheel ? 4 : 3;
    
    if (g_ps2MouseIndex < packetSize) return;
    g_ps2MouseIndex = 0;
    ...
    if (g_ps2MouseHasWheel)
    {
        rawZ = cast(byte)(g_ps2MousePacket[3] & 0x0F);
        // Sign extend 4-bit value
        if ((rawZ & 0x08) != 0) rawZ |= 0xF0;
        
        // FIX: If rawZ is -8 (0x08 nibble), it's likely a sync error where we read the next packet's header (0x08).
        // This happens if the mouse is sending 3-byte packets but we expect 4.
        // Ignore it to prevent ghost scrolling.
        if (rawZ == -8) rawZ = 0;
    }
    ...
    if (xNegative) deltaX = cast(int)(rawX | 0xFFFFFF00);
    if (yNegative) deltaY = cast(int)(rawY | 0xFFFFFF00);
    ...
    // Handle extra buttons if present
    if (g_ps2MouseHas5Buttons && g_ps2MouseHasWheel)
    {
        if ((g_ps2MousePacket[3] & 0x10) != 0) report.buttons |= 0x08; // Button 4
        if ((g_ps2MousePacket[3] & 0x20) != 0) report.buttons |= 0x10; // Button 5
    }
    
    report.deltaX = cast(byte)deltaX;
    report.deltaY = cast(byte)deltaY;
    
    // Z-axis is usually inverted for scroll wheel (positive = up/away)
    // But HID expects positive = up?
    // Serenity says: "Scroll only functions correctly when the sign is flipped there"
    // Let's try passing rawZ first.
    // Actually, standard PS/2 wheel: 0x01 (up), 0xFF (down).
    // Let's just pass it.
    report.deltaWheel = rawZ;
...
private void pollLegacyPS2(ref InputQueue queue) @nogc nothrow
{
    ...
    /*
    int loopLimit = 256;
    while ((inb(ps2StatusPort) & ps2StatusOutputFull) != 0 && loopLimit-- > 0)
    {
        const ubyte status = inb(ps2StatusPort);
        const ubyte data = inb(ps2DataPort);

        // Some controllers may not set the mouse bit; however, the heuristic
        // (data & 0x08) != 0 is too aggressive and eats keyboard keys (e.g. 'a' = 0x1E).
        // We rely on the status bit or the ISR queue.
        if ((status & ps2StatusIsMouse) != 0)
            handlePs2MouseByte(data, queue);
        else
            handlePs2KeyboardByte(data, queue);
