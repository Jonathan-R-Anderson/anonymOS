...
extern(C) void aes_encrypt(ubyte* data, const(ubyte)* key) {
    ...
    aes_encrypt_rk(data, rk.ptr);
}
extern(C) void aes_decrypt(ubyte* data, const(ubyte)* key) {
    ubyte[240] rk = void;
    aes256_expand(key, rk.ptr);
    aes_decrypt_rk(data, rk.ptr);
}
// Expand once, then encrypt/decrypt many blocks: the XTS sector loop used to re-expand the key
// schedule for EVERY 16-byte block (32 expansions per sector, 240 bytes each) -- the dominant cost
// of streaming an encrypted install at this kernel's optimisation level.
extern(C) void aes256_key_schedule(const(ubyte)* key, ubyte* rk240) { aes256_expand(key, rk240); }
extern(C) void aes_encrypt_rk(ubyte* data, const(ubyte)* rk) {
    ubyte[16] s = void;
...
// ── AES-256 DECRYPT (the inverse cipher, FIPS-197 §5.3) ───────────────────────
// The kernel had ENCRYPT only.  Design D's objstore decrypt path (xts_decrypt_sector
// in veracrypt_impl.d) needs the block inverse, and it MUST be byte-for-byte identical
// to the pre-boot loader's aes_dec (deps/veracrypt/efi/efi_vc.c) or the encrypted object
// store cannot be read back.  vcCryptoKat() below proves it against FIPS-197 and against
// a full XTS encrypt→decrypt round-trip.
...
// GF(2^8) multiply (for InvMixColumns coefficients 0x0e,0x0b,0x0d,0x09).
private ubyte gmul(ubyte a, ubyte b) {
    ubyte p = 0;
    foreach (_; 0 .. 8) {
        if (b & 1) p ^= a;
        ubyte hi = cast(ubyte)(a & 0x80);
        a = cast(ubyte)(a << 1);
        if (hi) a ^= 0x1b;
        b >>= 1;
    }
...
private void invShiftRows(ubyte* s) {
    ubyte t;
    t = s[13]; s[13]=s[9]; s[9]=s[5]; s[5]=s[1]; s[1]=t;                // row1 >>1
    t = s[2]; s[2]=s[10]; s[10]=t; t = s[6]; s[6]=s[14]; s[14]=t;      // row2 >>2 (== <<2)
    t = s[3]; s[3]=s[7]; s[7]=s[11]; s[11]=s[15]; s[15]=t;             // row3 >>3
}

private void invMixColumns(ubyte* s) {
    foreach (c; 0 .. 4) {
        ubyte* col = s + c * 4;
        ubyte b0 = col[0], b1 = col[1], b2 = col[2], b3 = col[3];
        col[0] = cast(ubyte)(gmul(b0,0x0e) ^ gmul(b1,0x0b) ^ gmul(b2,0x0d) ^ gmul(b3,0x09));
        col[1] = cast(ubyte)(gmul(b0,0x09) ^ gmul(b1,0x0e) ^ gmul(b2,0x0b) ^ gmul(b3,0x0d));
        col[2] = cast(ubyte)(gmul(b0,0x0d) ^ gmul(b1,0x09) ^ gmul(b2,0x0e) ^ gmul(b3,0x0b));
        col[3] = cast(ubyte)(gmul(b0,0x0b) ^ gmul(b1,0x0d) ^ gmul(b2,0x09) ^ gmul(b3,0x0e));
    }
}

// Decrypt one 16-byte block in place under `key` (32 bytes).  AES-256, 14 rounds.
extern(C) void aes_decrypt_rk(ubyte* data, const(ubyte)* rk) {
    ubyte[16] s = void;
    foreach (i; 0 .. 16) s[i] = cast(ubyte)(data[i] ^ rk[14 * 16 + i]);   // AddRoundKey(14)

    for (int round = 13; round >= 1; round--) {
        invShiftRows(s.ptr);                                   // InvShiftRows
        foreach (i; 0 .. 16) s[i] = INV_SBOX[s[i]];            // InvSubBytes
        foreach (i; 0 .. 16) s[i] ^= rk[round * 16 + i];       // AddRoundKey(round)
        invMixColumns(s.ptr);                                  // InvMixColumns
    }
    invShiftRows(s.ptr);                                       // final InvShiftRows
    foreach (i; 0 .. 16) s[i] = INV_SBOX[s[i]];                // final InvSubBytes
    foreach (i; 0 .. 16) data[i] = cast(ubyte)(s[i] ^ rk[i]);  // AddRoundKey(0)
}
...
public void vcCryptoKat() {
    ...
    // AES-256 DECRYPT (FIPS-197 inverse): ct 8ea2b7ca...4b496089 → pt 00112233..ff.
    // Proves aes_decrypt inverts aes_encrypt (the objstore read path depends on it).
    ubyte[16] dblk = aesWant;
    immutable ubyte[16] decWant = [
        0x00,0x11,0x22,0x33,0x44,0x55,0x66,0x77,0x88,0x99,0xaa,0xbb,0xcc,0xdd,0xee,0xff];
    aes_decrypt(dblk.ptr, key.ptr);
    foreach (i; 0 .. 16) if (dblk[i] != decWant[i]) ok = false;
