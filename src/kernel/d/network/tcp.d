...
export extern(C) int tcpSocket() @nogc nothrow {
    ...
    g_tcpConnections[sockfd].onAccept = null;
...
export extern(C) void tcpHandlePacket(const(ubyte)* data, size_t len,
    ...
    for (size_t i = 0; i < g_tcpConnectionCount; i++) {
        ...
        if (conn.localPort == destPort) {
            // Exact match
            if (conn.remotePort == srcPort && conn.remoteIP.isEqual(srcIP)) {
                sockfd = cast(int)i;
                break;
            }
            // Listen match (save for later if no exact match found)
            if (conn.state == TCPState.LISTEN) {
                listenSockfd = cast(int)i;
            }
        }
    }
    
    if (sockfd < 0) sockfd = listenSockfd;
    ...
    switch (conn.state) {
        case TCPState.LISTEN:
            if (flags & TCPFlags.SYN) {
                // Incoming connection - Spawn new socket
                int newSockfd = tcpSocket();
                if (newSockfd < 0) return; // No resources
                
                TCPConnection* newConn = &g_tcpConnections[newSockfd];
                newConn.localPort = conn.localPort;
                newConn.remoteIP = srcIP;
                newConn.remotePort = srcPort;
                newConn.recvSeq = seqNum + 1;
                newConn.sendAck = newConn.recvSeq;
                newConn.sendSeq = g_initialSeqNum++;
                newConn.sendWindow = windowSize;
                newConn.state = TCPState.SYN_RECEIVED;
                
                // Inherit callbacks (except onAccept)
                newConn.onConnect = conn.onConnect;
                newConn.onData = conn.onData;
                newConn.onClose = conn.onClose;
                
                // Notify listener
                if (conn.onAccept !is null) {
                    conn.onAccept(sockfd, newSockfd, srcIP, srcPort);
                }
                
                // Send SYN-ACK from new socket
                tcpSendPacket(newSockfd, TCPFlags.SYN | TCPFlags.ACK, null, 0);
            }
            break;
