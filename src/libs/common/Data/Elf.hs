...
elf64Interp :: Ptr Elf64Hdr -> IO (Maybe String)
elf64Interp elf64Hdr =
    do (_, progHdrs) <- elf64ProgHdrs elf64Hdr
       case foldr pickInterp Nothing progHdrs of
         Nothing -> return Nothing
         Just pHdr ->
             do let p = castPtr (elf64Hdr `plusPtr` fromIntegral (ph64Offset pHdr)) :: Ptr Word8
                    n = fromIntegral (ph64FileSz pHdr)
                s <- readCStringN p n
                return (Just s)
    where
      pickInterp pHdr acc =
          case (ph64Type pHdr, acc) of
            (PtInterp, Nothing) -> Just pHdr
            _ -> acc

      readCStringN _ 0 = return ""
      readCStringN p n =
          do c <- peek p
             if c == 0
                then return ""
                else do rest <- readCStringN (p `plusPtr` 1) (n - 1)
                        return (toEnum (fromIntegral c) : rest)
