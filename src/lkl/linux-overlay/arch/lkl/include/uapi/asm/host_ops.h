...
struct lkl_dev_pci_ops {
	...
	/* WiFi W1: return the MSI message field `field` (0=addr_lo,1=addr_hi,2=data) the
	 * arch MSI setup programs into the device's MSI capability. NULL = no MSI support. */
	long (*msi_setup)(struct lkl_pci_dev *dev, int field);
