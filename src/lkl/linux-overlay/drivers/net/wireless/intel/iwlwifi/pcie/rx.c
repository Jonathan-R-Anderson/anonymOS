...
void iwl_pcie_reset_ict(struct iwl_trans *trans)
{
	...
	/*
	 * EpinAnonymOS/LKL: keep ICT DISABLED in single-MSI mode so the legacy ISR reads interrupt
	 * causes straight from CSR_INT (iwl_pcie_int_cause_non_ict) instead of the DRAM ICT table.
	 * CSR_INT is a simple status/ack register (no auto-mask, one less DMA path) and lets our
	 * on-screen HUD observe the fw's ALIVE bit directly. (The ICT is an interrupt-coalescing
	 * optimisation we don't need for bring-up.)
	 */
...
#ifdef CONFIG_LKL
	/*
	 * EpinAnonymOS/LKL: all our MSI-X vectors share ONE IDT vector, and the host bridge
	 * (lkl-boot.c epin_irq_thread) raises EVERY registered wifi vector on each interrupt
	 * wake (it can't tell which one fired).  This handler CLEARS the FH/HW cause registers
	 * on entry (just above), so whichever fanout invocation runs first zeroes the causes and
	 * any later invocation reads 0 and early-returns below WITHOUT scheduling RX NAPI — so the
	 * firmware's UCODE_ALIVE_NTFY that the fw DMA'd into RXQ0 is never drained by
	 * iwl_pcie_rx_handle -> the alive wait times out (-110, "Failed to start RT ucode").
	 * Unconditionally kick RXQ0's NAPI here.  It self-limits: iwl_pcie_rx_handle no-ops when
	 * the read pointer already equals the hw write pointer, so a spurious kick is harmless,
	 * and the alive notification (and all subsequent RX) is guaranteed to be drained.
	 */
	if (trans_pcie->rxq && trans_pcie->rxq[0].napi.poll) {
		local_bh_disable();
		if (napi_schedule_prep(&trans_pcie->rxq[0].napi))
			__napi_schedule(&trans_pcie->rxq[0].napi);
		local_bh_enable();
