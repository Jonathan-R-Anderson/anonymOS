...
#ifdef CONFIG_LKL
	/*
	 * EpinAnonymOS/LKL: this AX210 LTR-ROM-erratum workaround MUST keep the CPU and
	 * device maximally busy (a tight poll) for ~100ms so the image loader / FSEQ RF
	 * power-up completes — otherwise the RT ucode's LMAC never starts (FSEQ_ERROR,
	 * LMAC PC=0xd0, alive -110).  But LKL is a single-cpu COOPERATIVE kernel: a busy-
	 * spin monopolises the one cpu so the timer IRQ can't run and JIFFIES FREEZES,
	 * making time_before(jiffies,end_time) loop forever (observed: 850k loops, clock
	 * frozen).  Fix: keep the tight busy-poll (no yield — the device stays busy) but
	 * bound it by a REAL-TIME ktime deadline; ktime_get() reads the host clock
	 * directly and keeps advancing even while we hold the cpu, so we spin the full
	 * 100ms of real time and then exit.  jiffies catches up once the cpu is freed.
	 */
	ktime_t iml_end = ktime_add_ms(ktime_get(), 100);
	(void)end_time;
#endif
...
#ifdef CONFIG_LKL
	while (ktime_before(ktime_get(), iml_end)) {
#else
