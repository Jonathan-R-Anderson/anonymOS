			      ...
			      const struct iwl_trans_config *trans_cfg)
{
	...
	/* EpinAnonymOS: we run iwlwifi in single-MSI (non-MSI-X) mode, so the gen2 FW-reset
	 * handshake's RESET_DONE cause (an MSI-X HW cause, read from CSR_MSIX_HW_INT_CAUSES_AD)
	 * is NEVER raised -> the handshake ALWAYS times out and then calls iwl_trans_fw_error(),
	 * which declares the firmware dead and aborts init (observed: "timeout waiting for FW
	 * reset ACK (inta_hw=0x0)" + NMI_INTERRUPT_UNKNOWN).  Force it off so gen2_stop_device
	 * falls back to the plain CSR reset, which uses the MSI+CSR_INT path that IS working. */
	trans_pcie->fw_reset_handshake = false;
}
