...
string[] builtinNames = [
    ...
    "rmdir", "tail", "touch", "unalias", "local", "locate", "forktest", "unshare"
];
...
// Stub readline implementation
extern(C) char* readline(const char* prompt) {
    import core.stdc.stdlib : malloc;
    import core.stdc.string : strcpy;
    import std.string : toStringz, stripRight, fromStringz;
    import std.stdio : write, readln;

    try {
        write(fromStringz(prompt));
        string line = readln();
        if (line is null) return null;
        line = line.stripRight("\n");
        char* ptr = cast(char*)malloc(line.length + 1);
        if (ptr == null) return null;
        strcpy(ptr, line.toStringz);
        return ptr;
    } catch (Exception e) {
        return null;
    }
...
void runCommand(string cmd, bool skipAlias=false, size_t callLine=0, string callFile="") {
    ...
    if(op == "command") {
        ...
        forktest.forktestCommand(tokens);
    } else if(op == "id") {
        id.idCommand(tokens);
    } else if(op == "unshare") {
        unshare.unshareCommand(tokens);
        ...
        writeln("gzip: not supported in minimal shell");
    } else if(op == "hash") {
        bool reset = false;
        string filename;
        size_t idx = 1;
