...
void objectsystemCommand(string[] tokens) {
    if(tokens.length < 2) {
        writeln("Usage: objectsystem [create|get|set|list|call|dump] ...");
        return;
    }
    string op = tokens[1];
    if (op == "dump") {
        foreach(k, v; registry) writeln(k, "\t", v.type);
    } else if (op == "create") {
        if(tokens.length < 3) return;
        writeln(createObject(tokens[2]));
    } else if (op == "get") {
        if(tokens.length < 4) return;
        writeln(getProp(tokens[2], tokens[3]));
    } else if (op == "set") {
        if(tokens.length < 5) return;
        setProp(tokens[2], tokens[3], tokens[4]);
    } else if (op == "call") {
         if(tokens.length < 4) return;
         string[] args = tokens.length > 4 ? tokens[4..$] : [];
         writeln(callMethod(tokens[2], tokens[3], args));
    }
}
