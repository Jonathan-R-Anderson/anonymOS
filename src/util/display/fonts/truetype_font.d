...
bool renderGlyph(ref TrueTypeFont font, dchar codepoint, ref ubyte[glyphWidth * glyphHeight] mask, out uint advance) @nogc nothrow
{
    ...
    // Error 64 (0x40) is FT_Err_Invalid_Glyph_Index - can happen under memory pressure
    FT_Error error = FT_Load_Glyph(font.ftFace, glyphIndex, FT_LOAD_RENDER | FT_LOAD_MONOCHROME | FT_LOAD_NO_HINTING);
    if (error != 0)
    {
        // Silently fail - font stack will fall back to bitmap font
        // Logging every failure causes console spam during heavy rendering
        return false;
    }
