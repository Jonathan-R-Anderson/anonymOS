...
X11StackState bootstrapX11Stack(X11StackConfig config) @nogc nothrow
{
    ...
    // Try internal X11 server first
    if (config.enableXorgServer)
    {
        if (startX11Server())
        {
            state.xorgServerReady = true;
        }
        else
        {
            // Fallback to external binary
            state.xorgServerReady = startStackProcess("/bin/Xorg");
        }
    }
...
private bool startDisplayManager(DisplayManagerKind kind) @nogc nothrow
{
    final switch (kind)
    {
        ...
        case DisplayManagerKind.gdm:
            return startStackProcess("/sbin/gdm");
    }
}
