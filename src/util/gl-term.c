// gl-term.c — R3: a GLES2-presented Wayland terminal (the GL twin of wl-term.c).
//
// Same engine as wl-term: an 80x30 character grid rendered with antialiased
// FreeType into a CPU framebuffer (a->pixels), keyboard input via wl_keyboard,
// scrollback, CSD titlebar + the per-domain identity border, hosting an
// interactive shell on a kernel PTY (/dev/ptmx + /dev/pts/N).  The ONLY
// difference: instead of sharing a->pixels via wl_shm, it uploads it as a GLES2
// texture and draws a fullscreen quad to an EGL window surface (eglSwapBuffers),
// so the client renders through the GL pipeline and Weston's GL renderer
// composites it.  Output is mirrored to stdout as "G4OUT: ..." for serial checks.
#define _GNU_SOURCE
...
// Render on the GPU when one is present: clear the four software-forcing env vars
// the kernel seeds into every program. (Until the virgl-client dmabuf path lands,
// EGL still resolves to softpipe, but the client runs the real GL pipeline.)
__attribute__((constructor(101)))
static void glterm_use_gpu(void) {
    unsetenv("LIBGL_ALWAYS_SOFTWARE"); unsetenv("GALLIUM_DRIVER");
    unsetenv("MESA_LOADER_DRIVER_OVERRIDE"); unsetenv("GBM_ALWAYS_SOFTWARE");
}
...
struct app {
    ...
    uint32_t             *pixels;            // CPU framebuffer (FreeType renders here)
    // GL present (replaces wl_shm): upload `pixels` as a texture + draw a quad.
...
static void commit(struct app *a) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, a->width, a->height, 0, GL_RGBA, GL_UNSIGNED_BYTE, a->pixels);
    glUseProgram(a->gl_prog);
    glUniform1i(a->gl_u_tex, 0);
    static const GLfloat quad[] = {   // pos.xy, uv.xy — uv.y=0 at top (texture row 0 = our top row)
        -1.f,  1.f, 0.f, 0.f,   -1.f, -1.f, 0.f, 1.f,
         1.f,  1.f, 1.f, 0.f,    1.f, -1.f, 1.f, 1.f,
    };
    glVertexAttribPointer(a->gl_a_pos, 2, GL_FLOAT, GL_FALSE, 4 * sizeof(GLfloat), quad);
    glEnableVertexAttribArray(a->gl_a_pos);
    glVertexAttribPointer(a->gl_a_uv, 2, GL_FLOAT, GL_FALSE, 4 * sizeof(GLfloat), quad + 2);
    glEnableVertexAttribArray(a->gl_a_uv);
    glDrawArrays(GL_TRIANGLE_STRIP, 0, 4);
    eglSwapBuffers(a->egl_dpy, a->egl_surf);   // attaches + commits the wl_surface
...
// ── GL present (EGL window surface + textured quad) ──────────────────────────
static GLuint gl_make_shader(GLenum type, const char *src) {
    GLuint s = glCreateShader(type);
    glShaderSource(s, 1, &src, NULL); glCompileShader(s);
    GLint ok = 0; glGetShaderiv(s, GL_COMPILE_STATUS, &ok);
    if (!ok) { char log[256]; glGetShaderInfoLog(s, sizeof log, NULL, log); printf("G4TERM: shader: %s\n", log); }
    return s;
}

static int create_gl_surface(struct app *a) {
    a->pixels = malloc((size_t)a->width * (size_t)a->height * 4);
    if (!a->pixels) { perror("G4TERM: malloc pixels"); return -1; }

    PFNEGLGETPLATFORMDISPLAYEXTPROC getpd =
        (PFNEGLGETPLATFORMDISPLAYEXTPROC)eglGetProcAddress("eglGetPlatformDisplayEXT");
    a->egl_dpy = getpd ? getpd(EGL_PLATFORM_WAYLAND_EXT, a->display, NULL)
                       : eglGetDisplay((EGLNativeDisplayType)a->display);
    EGLint maj, min;
    if (!eglInitialize(a->egl_dpy, &maj, &min)) { printf("G4TERM: eglInitialize 0x%x\n", eglGetError()); return -1; }
    eglBindAPI(EGL_OPENGL_ES_API);
    EGLint ca[] = { EGL_SURFACE_TYPE, EGL_WINDOW_BIT, EGL_RENDERABLE_TYPE, EGL_OPENGL_ES2_BIT,
                    EGL_RED_SIZE, 8, EGL_GREEN_SIZE, 8, EGL_BLUE_SIZE, 8, EGL_ALPHA_SIZE, 8, EGL_NONE };
    EGLint n = 0;
    if (!eglChooseConfig(a->egl_dpy, ca, &a->egl_cfg, 1, &n) || n < 1) { printf("G4TERM: eglChooseConfig failed\n"); return -1; }
    EGLint xa[] = { EGL_CONTEXT_CLIENT_VERSION, 2, EGL_NONE };
    a->egl_ctx = eglCreateContext(a->egl_dpy, a->egl_cfg, EGL_NO_CONTEXT, xa);
    if (a->egl_ctx == EGL_NO_CONTEXT) { printf("G4TERM: eglCreateContext failed\n"); return -1; }

    a->egl_window = wl_egl_window_create(a->surface, a->width, a->height);
    a->egl_surf = eglCreateWindowSurface(a->egl_dpy, a->egl_cfg, (EGLNativeWindowType)a->egl_window, NULL);
    if (a->egl_surf == EGL_NO_SURFACE) { printf("G4TERM: eglCreateWindowSurface 0x%x\n", eglGetError()); return -1; }
    eglMakeCurrent(a->egl_dpy, a->egl_surf, a->egl_surf, a->egl_ctx);

    GLuint vs = gl_make_shader(GL_VERTEX_SHADER,
        "attribute vec2 pos; attribute vec2 uv; varying vec2 vUV;"
        "void main(){ vUV = uv; gl_Position = vec4(pos, 0.0, 1.0); }");
    GLuint fs = gl_make_shader(GL_FRAGMENT_SHADER,
        // a->pixels is XRGB8888 (little-endian bytes B,G,R,X); GL_RGBA reads them as
        // (B,G,R,X) so swizzle .bgr back to (R,G,B) and force opaque alpha.
        "precision mediump float; varying vec2 vUV; uniform sampler2D tex;"
        "void main(){ gl_FragColor = vec4(texture2D(tex, vUV).bgr, 1.0); }");
    a->gl_prog = glCreateProgram();
    glAttachShader(a->gl_prog, vs); glAttachShader(a->gl_prog, fs); glLinkProgram(a->gl_prog);
    a->gl_a_pos = glGetAttribLocation(a->gl_prog, "pos");
    a->gl_a_uv  = glGetAttribLocation(a->gl_prog, "uv");
    a->gl_u_tex = glGetUniformLocation(a->gl_prog, "tex");
